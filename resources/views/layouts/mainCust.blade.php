                      ...
                      <li class="nav-item dropdown no-arrow">
                        <div class="nav-item dropdown no-arrow">
                          <a class="dropdown-toggle nav-link" aria-expanded="false" data-bs-toggle="dropdown" href="#">
                            <span class="d-none d-lg-inline me-2 text-gray-600 small">{{ Auth::guard('customer')->user()->nama }}</span>
                            @if (Auth::guard('customer')->user()->img == NULL)
                              <img class="border rounded-circle img-profile" src="{{ url('assets/img/avatars/avatar6.jpeg') }}"/>
                            @else
                              <img class="border rounded-circle img-profile" src="{{ url('fotocust/',Auth::guard('customer')->user()->img) }}"/>
                            @endif
                          </a>
                          <div class="dropdown-menu shadow dropdown-menu-end animated--grow-in">
                            <a class="dropdown-item" href="{{ route('customer.logoutC') }}" onclick="event.preventDefault();document.getElementById('logout-form').submit();">
                              <i class="fas fa-sign-out-alt fa-sm fa-fw me-2 text-gray-400"></i>&nbsp;Logout
                            </a>
                            <form action="{{ route('customer.logoutC') }}" id="logout-form" method="post">@csrf</form>
                          </div>
                        </div>
                      </li>
                    </ul>
